...
namespace Lastik.Views.Controls.TextOnPath
{
    public class TextOnPath : FrameworkElement
    {
        ...
        private void CreatePaintGeometry()
        {
            ...
            var pathBounds = (Path as Geometry).Bounds;
            paintGeometry = new PathGeometry();
            paintGeometry.FillRule = FillRule.Nonzero;
            var charsWidth = 0.0;
            
            foreach (var c in formattedChars)
            {
                charsWidth += c.WidthIncludingTrailingWhitespace;
            }
            var startProgress = (pathLength - charsWidth) / 2 / pathLength;
            ...
            points.Add(pathGeometry.Figures.First().StartPoint);
            foreach (var segment in pathGeometry.Figures.First().Segments)
            {
                switch (segment)
                {
                    case LineSegment line:
                        ...
                        break;
                    case PolyBezierSegment poly:
                        points.AddRange(poly.Points);
                        break;
                }
            }

            var centerY = points[points.Count / 2].Y;
            if (points.Count % 2 == 0)
            {
                centerY += points[(points.Count - 1) / 2].Y;
                centerY /= 2.0;
            }
            var offsetY = 0.0;
            if (points.First().Y < points.Last().Y)
            {
                offsetY = centerY - points.First().Y;
            }
            else
            {
                offsetY = centerY - points.Last().Y;
            }

            offsetY *= startProgress * 2;
            RenderTransform = new TranslateTransform(pathBounds.Left + pathBounds.Width * startProgress, pathBounds.Top + offsetY);
        }
